...
const int Beep_Start[] = {700, 900, 1100};  // 낮은음 → 높은음
const int Beep_Stop[] = {1100, 900, 700};  // 높은음 → 낮은음
volatile int beep_length = 0;  // 3단계 소리
volatile int beep_active = 0;   // 부저 활성화 여부
volatile int beep_step = 0;     // 현재 재생 중인 음
volatile const int *beep_sequence; 

extern volatile int buzzer_msec_count;
...
{
	DDRB |= 1 << 5;   // PWM CHANNEL  OC3A(PE3) 출력 모드로 설정 한다. 
...
void Beep()
{
	beep_length = 1;
	OCR1A = 2000;
	buzzer_msec_count = 200; // 200ms 동안 유지 (1ms 주기당 2 카운트)
	beep_step = 0;
	beep_active = 1;
...
void start_sound(){
	beep_sequence = Beep_Start;
	beep_length = 3;
	buzzer_msec_count = 200;
	beep_step = 0;
	beep_active = 1;
	OCR1A = beep_sequence[beep_step];
}

void stop_sound(){
	beep_sequence = Beep_Stop;
	beep_length = 3;
	buzzer_msec_count = 200;
	beep_step = 0;
	beep_active = 1;
